...
#' @name RMLA
#' @description It estimates the RMLA, the square root of the systematic error 
...
#' @details The RMLA represents the systematic (bias) component of the MSE
...
#' @export 
RMLA <- function(data=NULL,
                obs,
                pred,
                tidy = FALSE,
                na.rm = TRUE){
  RMLA <- rlang::eval_tidy(
    data = data,
    rlang::quo(
    sum ((
  ...
  if (tidy==TRUE){ return(as.data.frame(RMLA)) }
  
  if (tidy==FALSE){ return(list("RMLA" = RMLA)) }
}