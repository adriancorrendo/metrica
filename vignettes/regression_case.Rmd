...
In some cases, we may count with time-series predictions. For example, let's say that we evaluate the production of drymass during the season. For this specific case, the Mean Absolute Scaled Error is a more solid metric compared to conventional RMSE or similar metrics.

```{r metrics time-series}

# Let's suppose that predictions of drymass on the chickpea dataset were made during the cropping season
# So in this case, we may create a new variable called time that will serve to sort the observations
chickpea_time <- metrica::chickpea %>% dplyr::arrange(pred) %>% 
  mutate(time = seq(1,39, by =1)) %>% 
  mutate(obs = pred *1.1 + rnorm(n=39, mean = 0, sd = 60))

# Plot
chickpea_time %>% ggplot2::ggplot(aes(x = time))+
  ...
  labs(x = "Days after planting", y = "Chickpea Drymass (kg/ha)")+
  theme_bw()+
  theme(legend.position = "top")

# MASE estimate, with naive approach (random-walk, i.e. using observation of t-1 as prediction)
metrica::MASE(data = chickpea_time, obs = obs, pred = pred, 
              naive_step = 1, tidy = FALSE, time = time)

# MASE estimate, with mae coming from an independent training set.
metrica::MASE(data = chickpea_time, obs = obs, pred = pred, 
              naive_step = 1, tidy = FALSE, time = time, oob_mae = 120)
