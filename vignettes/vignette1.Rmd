...
The user can also create a scatter plot that includes not only the **predicted** vs. **observed** points, **1:1 line**, and **regression line**, but also **5 selected metrics and their values** and the **regression equation**.  

This is acompplished with the function `scatter.metrics()`:  

```{r scatter.metrics}
scatter.metrics(data = wheat,
   obs = obs, 
   pred = pred)
```
...
```{r export metric.summary, eval=F }
metrics.summary(data = wheat,
   obs = obs, 
   pred = pred) %>%
